...
u8 gt9xx_sensor_id = 0xff;

static ssize_t touch_config_proc_write(struct file *file,
				      const char __user *buffer,
				      size_t count, loff_t *pos);
static int touch_config_proc_open(struct inode *inode, struct file *file);
static struct list_head gt_nd_struct_list;
static const struct file_operations info_proc_file_ops = {
	.owner	 = THIS_MODULE,
    .open    = touch_config_proc_open,
    .read	 = seq_read,
	.llseek	 = seq_lseek,
	.release = seq_release,
	.write   = touch_config_proc_write,
};
...
#if defined(ZTE_FEATURE_TOUCH_GT615)
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, 1280-y);//wkh add
#else
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    //if (DOZE_ENABLED == doze_status)
    if (DOZE_DISABLED != doze_status && ts->enable_gesture_wakeup)
...
//add by yujianhua begin
static void touch_unregister_proc(void)
{
    if (touch_proc_dir_entry) {
		remove_proc_entry("touchscreen/ts_information", NULL);
		remove_proc_entry("touchscreen", NULL);
		touch_proc_dir_entry = NULL;
	}
}

static void *touch_config_proc_start(struct seq_file *m, loff_t *pos)
{
	return seq_list_start_head(&gt_nd_struct_list, *pos);
}

static void *touch_config_proc_next(struct seq_file *p, void *v, loff_t *pos)
{
	return seq_list_next(v, &gt_nd_struct_list, pos);
}

static void touch_config_proc_stop(struct seq_file *m, void *v)
{
}

static int touch_config_proc_show(struct seq_file *m, void *v)
{
       s32 ret = -1;

	u8 buf[8] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};

	seq_puts(m, "Manufacturer:goodix\n");

	seq_printf(m, "client_addr:0x%x\n", i2c_connect_client ->addr);	
	...
	{        
		...
		//chip info	
		seq_printf(m, "chip info:%c%c%c\n",buf[2], buf[3], buf[4]); 
	}
	else
	{
		//GTP_INFO("IC VERSION:%c%c%c%c_%02x%02x\n",	buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
		//chip info
		seq_printf(m, "chip info:%c%c%c%c\n",buf[2], buf[3], buf[4], buf[5]);
	}
	seq_printf(m, "firmware version: %02x%02x\n", buf[7], buf[6]);   
    
	//module info
	switch (gt9xx_sensor_id){
#if defined(CONFIG_GT9XX_TOUCHPANEL_P839T60)
	case 0x0:
		seq_printf(m,  "module : %s (0x%x)\n", "GT9XX + yeji", grp_cfg_version);//junda
		break;
	case 0x2:	
		seq_printf(m,  "module : %s (0x%x)\n", "GT9XX + junda", grp_cfg_version);//yeji
		break;
#else
	case 0x0:
	case 0x6:
...
#endif
	default:
		seq_printf(m,  "module : %s (0x%x)\n", "GT9XX + unknown", grp_cfg_version);
		break;
	}
#if defined(CONFIG_GT9XX_TOUCHPANEL_P839T60)
#else
    //differ black or white
	if(gt9xx_sensor_id > 5)
	{
	    seq_puts(m, "white TP\n");
	}
	else
...
static const struct seq_operations proc_config_ops = {
	.start = touch_config_proc_start,
	.next  = touch_config_proc_next,
	.stop  = touch_config_proc_stop,
	.show  = touch_config_proc_show,
};
static int touch_config_proc_open(struct inode *inode, struct file *file)
{
...
// add by guohaijing start
static ssize_t goodix_get_palm_sleep(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct goodix_ts_data *drvdata= dev_get_drvdata(dev);
    int enable = drvdata->enable_palm_sleep;
    return snprintf(buf, sizeof(enable) + 2, "%d\n", enable);
}

static ssize_t goodix_set_palm_sleep(struct device *dev, struct device_attribute *attr,
                             const char *buf, size_t size)
{
    ...
    drvdata->enable_palm_sleep = (int)enable;
...
{
    struct goodix_ts_data *drvdata= dev_get_drvdata(dev);
    int enable = drvdata->enable_gesture_wakeup;
    return snprintf(buf, sizeof(enable) + 2, "%d\n", enable);
}
                             ...
                             const char *buf, size_t size)
{
    ...
    drvdata->enable_gesture_wakeup = (int)enable;

    return size;
}

/*static ssize_t goodix_get_doze_status(struct device *dev, struct device_attribute *attr, char *buf)
{
    int enable = doze_status;
    return snprintf(buf, sizeof(enable) + 2, "%d\n", enable);
}

static ssize_t goodix_set_doze_status(struct device *dev, struct device_attribute *attr,
                             const char *buf, size_t size)
{
    struct goodix_ts_data *drvdata= dev_get_drvdata(dev);
    unsigned long enable = 0;
	...
	if (drvdata->pdata->dbl_clk_wakeup && drvdata->enable_gesture_wakeup && DOZE_ENABLED == enable) {
		gtp_enter_doze(drvdata);
...
#if defined(CONFIG_GT9XX_TOUCHPANEL_P839T60)
#else
static ssize_t goodix_get_sensor_id(struct device *dev, struct device_attribute *attr, char *buf)
{
    int sensor_tmp = 0;
	if(gt9xx_sensor_id > 5)//white
	{
	    sensor_tmp = 1;
	}
	else//black
	...
	return snprintf(buf, sizeof(sensor_tmp) + 2, "%d\n", sensor_tmp);
}

static ssize_t goodix_set_sensor_id(struct device *dev, struct device_attribute *attr,
                             const char *buf, size_t size)
{
    return size;
}
...
//static DEVICE_ATTR(enable_doze_status, 0664, goodix_get_doze_status, goodix_set_doze_status);
// add by guohaijing end
#if defined(CONFIG_GT9XX_TOUCHPANEL_P839T60)
#else
...
//add by yujianhua begin
    touch_proc_dir_entry = proc_mkdir("touchscreen", NULL);
	if (!touch_proc_dir_entry)
        dev_info(&client->dev, "touch_proc_dir_entry is null\n");
		//return;
    proc_create("touchscreen/ts_information", 0644, NULL, &info_proc_file_ops);	
...
//add by yujianhua end
    ...
    // add by guohaijing start
    ret = device_create_file(&client->dev, &dev_attr_enable_palm_sleep);
    if (ret) {
        dev_err(&client->dev, "GTP create dev_attr_enable_palm_sleep failed\n");
        device_remove_file(&client->dev, &dev_attr_enable_palm_sleep);
    }

    ret = device_create_file(&client->dev, &dev_attr_enable_gesture_wakeup);
    if (ret) {
        dev_err(&client->dev, "GTP create dev_attr_enable_gesture_wakeup failed\n");
        device_remove_file(&client->dev, &dev_attr_enable_gesture_wakeup);
    }

    /*ret = device_create_file(&client->dev, &dev_attr_enable_doze_status);
    if (ret) {
        dev_err(&client->dev, "GTP create dev_attr_enable_doze_status failed\n");
        device_remove_file(&client->dev, &dev_attr_enable_doze_status);
    }*/
    // add by guohaijing end
#if defined(CONFIG_GT9XX_TOUCHPANEL_P839T60)
#else
    ret = device_create_file(&client->dev, &dev_attr_info_sensor_id);
    if (ret) {
...
#endif
    touch_unregister_proc();//add by yujianhua
    if (ts) 
    {
        if (ts->use_irq)
        {
...
//		mutex_unlock(&ts->lock);
    ...
    if (ts->pdata->dbl_clk_wakeup && ts->enable_gesture_wakeup) {
