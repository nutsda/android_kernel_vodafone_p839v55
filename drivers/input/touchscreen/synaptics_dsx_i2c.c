...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	if (of_property_read_bool(np, "synaptics,vdd-io-gpio")) {
	...
	if (of_property_read_bool(np, "synaptics,vcc-dc-gpio")) {
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retval != PAGE_SELECT_LEN) {
				...
				dev_err(rmi4_data->pdev->dev.parent,
						"%s@%d: I2C send (0x%x) retry %d\n",
						__func__, __LINE__,(unsigned int)buf,retry + 1);
			#endif
			    dev_err(rmi4_data->pdev->dev.parent,
						"%s@%d: I2C send retry %d\n",
