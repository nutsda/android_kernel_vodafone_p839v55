 ...
 *
 * This driver is based on code found in a ZFG_ENV patch that modified
...
static int zfg_ev_usb_serial_open(struct tty_struct *tty,
...
static void zfg_ev_usb_serial_close(struct usb_serial_port *port)
{
	struct usb_device *udev = port->serial->dev;
	struct device *dev = &port->dev;
	int result = 0;
...
static struct usb_serial_driver zio_device = {
	...
	.id_table =		id_table,
	.num_ports =		1,
	.open =			zfg_ev_usb_serial_open,
	.close =		zfg_ev_usb_serial_close,
};
